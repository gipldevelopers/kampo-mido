...
export default function AddOffer() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ message: error.response?.data?.message || "Failed to create offer", type: "error" });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <div>
          <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold tracking-tight text-foreground flex items-center gap-2">
            <Plus className="text-primary w-5 h-5 sm:w-6 sm:h-6" /> Create New Offer
          </h2>
          <p className="text-[10px] sm:text-xs md:text-sm text-muted-foreground mt-0.5">Define a new promotional coupon for your customers.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4 sm:space-y-6">
          <form onSubmit={handleSubmit} className="bg-card border border-border rounded-lg sm:rounded-xl shadow-sm overflow-hidden flex flex-col">
            ...
            <div className="p-4 sm:p-6 bg-muted/10 border-t border-border flex justify-end gap-3">
               ...
               >
                 {loading ? <Loader2 size={16} className="animate-spin" /> : <><Plus size={16} /> Create Offer</>}
               </button>
            </div>
          </form>
        </div>
