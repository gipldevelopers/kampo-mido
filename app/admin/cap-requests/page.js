...
export default function CapRequestManagement() {
                  ...
                  <tr key={req.id} className="hover:bg-slate-50/30 transition-colors">
                    ...
                    <td className="px-3 py-5 text-slate-500">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">{new Date(req.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              <div className="grid grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">KYC Status</label>
                    <p className="text-sm font-semibold text-emerald-600">{selectedRequest.customer?.kycStatus?.toUpperCase()}</p>
                  </div>
                </div>
              </div>
              ...
              {selectedRequest.proofFile && (
                <div>
                  <label className="text-[10px] text-gray-400 uppercase font-bold tracking-widest mb-3 block">Supporting Evidence</label>
              ...
              {selectedRequest.status.toLowerCase() === 'pending' && (
                <div className="pt-6 border-t border-gray-100 space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-gray-400 uppercase">Approval/Rejection Notes</label>
                    <textarea
              ...
              {selectedRequest.status.toLowerCase() !== 'pending' && (
                <div className="pt-4 border-t border-gray-100">
                  <div className="p-4 rounded-xl bg-slate-50 text-slate-500 text-xs italic">
                    Processed by {selectedRequest.processor?.firstname || 'Admin'} with notes: "{selectedRequest.adminNotes || 'None'}"
                  </div>
                </div>
              )}
            </div>
